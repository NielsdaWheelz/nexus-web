...
const FG_ONLIGHT = "#7d5e35";
const BG_DARK = "#15140f";
const BG_LIGHT = "#faf8f3";
...
function makeBrandSvg(masterDots, fill) {
  const circles = masterDots
    .map((d) => `  <circle cx="${d.x}" cy="${d.y}" r="${d.r}" fill="${fill}"/>`)
    .join("\n");
  return `<?xml version="1.0" encoding="UTF-8"?>
...
function makeFaviconSvg(masterDots) {
  const circles = masterDots
    .map((d) => `  <circle class="dot" cx="${d.x}" cy="${d.y}" r="${d.r}"/>`)
    .join("\n");
  return `<?xml version="1.0" encoding="UTF-8"?>
...
function makeBrandTs(masterDots) {
  const dotLines = masterDots
    .map((d) => `  { cx: ${d.x}, cy: ${d.y}, r: ${d.r} },`)
    .join("\n");
  return `// Auto-generated by scripts/build-icons.mjs from apps/web/public/brand/asterism.svg.
...
function makeBrandCss() {
  return `/* Auto-generated by scripts/build-icons.mjs. Do not edit by hand. */
:root {
  --brand-fg-on-dark: ${FG_GOLD};
  --brand-fg-on-light: ${FG_ONLIGHT};
  --brand-bg-dark: ${BG_DARK};
  --brand-bg-light: ${BG_LIGHT};
}
`;
}
...
writeText(
  "apps/web/public/brand/asterism-onlight.svg",
  makeBrandSvg(masterDots, FG_ONLIGHT),
);
writeText(
  "apps/web/public/brand/asterism-ondark.svg",
  makeBrandSvg(masterDots, FG_GOLD),
);
writeText("apps/web/src/app/icon.svg", makeFaviconSvg(masterDots));
writeText("apps/web/src/lib/brand.ts", makeBrandTs(masterDots));
writeText("apps/web/src/app/brand.css", makeBrandCss());
