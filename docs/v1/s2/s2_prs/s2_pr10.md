...
## Status Note (2026-03 hardening follow-up)

This document is still the canonical reference for aligned mode behavior, but
reader hardening since PR-10 added EPUB-specific extensions:

- Explicit EPUB highlight scope toggle:
  - `This chapter` -> aligned mode (anchor-coupled rows)
  - `Entire book` -> list mode (chapter-indexed list)
- Media-wide highlight listing endpoint for book scope:
  - `GET /media/{media_id}/highlights` (cursor pagination)
- List-mode virtualization in `LinkedItemsPane` for large book highlight sets.
- EPUB E2E regression coverage for:
  - reload alignment/order stability
  - cross-chapter linked-item click -> chapter navigation -> focus landing
...
```ts
const ROW_HEIGHT = 44;          // px, fixed row height (two-line: text + annotation)
...
- Fixed height: `ROW_HEIGHT = 44px` (two-line: text + annotation)
