...
1. Read `data-highlight-ids` and `data-highlight-top`
2. First click on a segment focuses `data-highlight-top`
3. Subsequent clicks on **the same segment element** cycle through `data-highlight-ids` in order

### 9.2 Segment Identity

A "segment" is identified by the DOM element with `data-highlight-ids`:
