# PR-08 — Frontend Read-Only Highlight Rendering (L3 Spec)
...
2. Fetch highlights → normalize → run segmenter
3. Build canonical cursor by walking DOM with token stream (see §5)
...
7. Serialize transformed DOM → render via `dangerouslySetInnerHTML`
...
- Use `<span>` (not `<mark>`)
- Attributes:
  - `data-active-highlight-ids="{activeIds.join(" ")}"` — **space-delimited**, preserves segmenter ordering
  - `data-highlight-top="{topmostId}"`
- Class:
  - `.hl-{topmostColor}` where color ∈ {yellow, green, blue, pink, purple}

**Reason:**
- Avoid browser default `<mark>` styles
- Full visual control
- Space-delimited IDs enable efficient `~=` attribute selector (token match) for hover/focus
- `data-active-highlight-ids` ordering matches segmenter for consistent behavior
...
1. Find first segment where `highlight.id ∈ segment.activeIds`
...
If canonical text validation fails (§5.5):
...
5. **Codepoint length accounting for astral characters:**
   - Input HTML with emoji (e.g., `<p>Hello 🎉 World</p>`)
...
1. Invalid highlight (out of bounds) — skipped, warning logged
2. Canonical mismatch — all highlights skipped, plain HTML rendered
...
apps/web/src/lib/highlights/
  ├── canonicalCursor.ts       # buildCanonicalCursor()
  ├── canonicalCursor.test.ts
  ├── applySegments.ts         # applySegmentsToDom()
  ├── applySegments.test.ts
  └── highlights.module.css    # .hl-* styles

apps/web/src/components/
  └── HtmlRenderer.tsx         # Modified to accept pre-rendered HTML with highlights
```
...
Or a single prop with the transformed HTML. Implementation detail, but the component must not perform highlight application itself—that happens upstream.
