...
- **acceptance**:
  ...
  - `pr-04` currently plans to satisfy exact PDF duplicate race safety via service-level transactional enforcement using canonical geometry identity plus a transaction-scoped advisory lock + duplicate recheck (no schema/index refinement required for S6 unless implementation proves necessary).
  - `pr-04` currently plans deterministic canonical geometry identity byte serialization + SHA-256 lowercase hex `geometry_fingerprint`, and a stable namespaced deterministic `int64` advisory-lock key for duplicate locking (no Python `hash()`); duplicate recheck remains authoritative for correctness.
  - `pr-04` currently plans lock ordering for write-time PDF mutations as `media-scoped coordination lock` then `duplicate-identity advisory lock`; pure geometry canonicalization + shared PDF quote matching run before duplicate-lock acquisition, and duplicate-lock scope is bounded to duplicate recheck + atomic persistence.
  - `pr-04` may include a narrow `pr-03` interop patch so internal PDF text rebuild/invalidation paths adopt the shared media-scoped coordination lock contract; do not redesign `pr-03` lifecycle behavior as part of this interop fix.
  - `pr-04` currently plans a shared low-level `pdf_locking.py` helper for media-scoped coordination lock key derivation and lock-order helpers, reused by `pdf_highlights.py` and the narrow `pr-03` interop patch; do not duplicate coordination-lock namespace/order logic across modules.
  - `pr-04` currently plans to keep canonical duplicate-identity + duplicate-lock key derivation in `pdf_highlight_geometry.py` (with `pdf_locking.py` limited to coordination-lock keys and lock mechanics); do not redefine canonical duplicate identity in `pdf_locking.py`.
  ...
  - `pr-04` implements PDF geometry canonicalization/fingerprinting/sort-key derivation in a dedicated pure backend geometry module (current planned shape: `pdf_highlight_geometry.py`) and keeps transactional persistence/coherence/duplicate enforcement in highlight service orchestration; do not embed canonicalization math in route handlers or duplicate it across create/update codepaths.
  ...
  - `pr-04` introduces anchor-discriminated typed highlight responses for generic highlight routes and PDF highlight routes while preserving fragment-route response compatibility (including legacy fragment response fields where required by existing fragment-route contracts/tests); do not force a fragment-route response migration in `pr-04`.
  - `pr-04` currently plans a dedicated `pdf_highlights.py` backend orchestration module for PDF highlight create/list/update logic, while `highlights.py` remains the generic/fragment compatibility layer and route-facing integration surface.
  - `pr-04` owns the normative S6 write-path PDF match metadata + `prefix/suffix` storage behavior on create/bounds-update when quote-text infrastructure is ready (with `pending` + empty `prefix/suffix` fallback only when not ready); `pr-05` owns quote rendering usage and follow-on enrichment paths, not the core write-time storage contract.
  - `pr-04` and `pr-05` share a single pure deterministic PDF quote-match helper module (current planned shape: `pdf_quote_match.py`) for S6 `plain_text_match_version=1` matching and `prefix/suffix` derivation; do not duplicate the matching algorithm across write-time persistence and quote/enrichment services.
  - `pr-04` currently plans a classified hybrid write-time matcher anomaly policy on quote-ready media: recoverable/classified matcher anomalies degrade to `plain_text_match_status='pending'` with cleared offsets/version and empty `prefix/suffix` plus structured anomaly logging, while unexpected/unclassified matcher exceptions fail the mutation (`500`) with no partial write.
  - `pr-04`/`pr-05` currently plan a two-layer matcher anomaly boundary: pure `pdf_quote_match.py` provides typed recoverable anomaly classifications, while shared `pdf_quote_match_policy.py` owns structured anomaly logging + path-specific mapping helpers; do not move logging into the matcher or duplicate anomaly log schemas in service modules.
  - `pr-04`/`pr-05` currently plan a canonical matcher anomaly observability contract in `pdf_quote_match_policy.py`: one `pdf_quote_match_anomaly` event schema with centralized helper logging/mapping ownership and an explicit no-double-logging caller rule.
  - `pr-04`/`pr-05` currently plan a strict MVP no-content matcher anomaly logging policy in `pdf_quote_match_policy.py`: no raw document-derived text (`exact`, `prefix/suffix`, `media.plain_text` excerpts), no unsalted text hashes, and sanitized exception diagnostics with approved non-content fields only.
  - `pr-04` currently plans a backward-compatible unified generic `PATCH /highlights/{id}` request schema that preserves existing fragment fields and adds nested `pdf_bounds` for PDF bounds replacement, with strict mutual exclusivity between fragment offset updates and `pdf_bounds`.
  - `pr-04` currently plans service-level semantic anchor-kind mismatch rejection for cross-kind but syntactically valid unified PATCH payloads (`pdf_bounds` on fragment highlights or fragment offsets on PDF highlights): `400 E_INVALID_REQUEST` with deterministic anchor-kind mismatch details and no mutation after visibility/resource resolution.
  - `pr-04` currently plans self-same canonical PDF bounds-updates (same canonical geometry identity as target highlight) to be non-conflict PATCH successes: duplicate checks exclude the target id, and write-time derived fields may still recompute/persist when mutable inputs (e.g., `exact`) change.
  - `pr-04` currently plans fully identical effective PDF PATCH updates (including self-same canonical geometry and no effective field changes) as true no-op successes: `200` with no persisted-state mutation, no `updated_at` bump, and no quad/subtype row rewrites.
  - `pr-04` currently allows an optional pre-`D09` no-op short-circuit for fully identical effective PDF PATCH updates only under guarded conditions (transaction-scoped target-row lock + safe equality proof); otherwise it uses normal `D09` sequencing while preserving the same `D18` no-op semantics.
  - `pr-04` currently plans one canonical deterministic side-effect-free effective-state comparison helper/path (in `pdf_highlights.py` or an equivalent narrowly scoped helper in the same service boundary) reused by guarded pre-lock short-circuit checks and post-lock no-op detection, with an explicit structured fallback outcome when safe equality cannot be proven and the normal `D09` path must be used.
...
- **acceptance**:
  ...
  - `pr-05` reuses the shared pure deterministic PDF quote-match helper module introduced for `pr-04` (current planned shape: `pdf_quote_match.py`) for enrichment/repair/quote-path matching semantics; do not introduce quote-path-specific matcher drift.
  - `pr-05` reuses the shared `pdf_quote_match_policy.py` anomaly logging/mapping helpers introduced with `pr-04` (or their finalized equivalent under the same boundary) so recoverable/unclassified matcher anomaly handling and structured event schemas do not drift between write paths and quote/enrichment paths.
  - `pr-05` preserves the `pr-04` canonical `pdf_quote_match_anomaly` event schema and no-double-logging helper ownership contract when reusing `pdf_quote_match_policy.py`.
  - `pr-05` preserves the `pr-04` no-content matcher anomaly logging/redaction policy when reusing `pdf_quote_match_policy.py` unless a later security/privacy decision explicitly changes it.
  ...
  - `pr-05` currently plans a selective quote-path renderer error propagation contract: `render_context_blocks(...)` preserves log+skip behavior for generic/non-blocking context-render failures, but re-raises typed quote-blocking renderer errors (at minimum deterministic PDF readiness failures such as `E_MEDIA_NOT_READY`) so sync and streaming quote-to-chat paths surface the same S6-required user-visible errors instead of silently dropping requested PDF quote context.
  - `pr-05` currently plans a dedicated neutral shared quote-context error module (current planned shape: `quote_context_errors.py`) defining a transport-agnostic typed quote-blocking error vocabulary with API-error-code-safe metadata for renderer/context-rendering/sync-stream parity; do not raise transport-formatted `ApiError`/SSE payloads directly from PDF quote render helpers.
  - `pr-05` currently plans a minimal renderer-originating quote-blocking taxonomy: user-facing quote-blocking renderer errors are limited to deterministic PDF quote-not-ready (`E_MEDIA_NOT_READY`); recoverable matcher/coherence issues remain degrade-safe and unexpected renderer/validator/policy failures use internal-error parity unless a later explicit decision expands the blocking taxonomy.
  - `pr-05` currently plans request-blocking semantics for typed quote-blocking renderer errors: mixed-context quote-to-chat requests/streams fail deterministically on the first quote-blocking renderer error (e.g. `E_MEDIA_NOT_READY`) rather than silently dropping the blocking context for partial success.
  - `pr-05` currently plans a pre-provider quote-render gate for streaming: quote-context rendering (including typed quote-blocking checks) completes before provider token streaming starts, so a quote-blocking stream failure may follow `meta` but emits no `delta` events before deterministic terminal `done` error; no quote-specific recovery protocol is introduced in S6.
  - `pr-05` currently plans a single external internal-failure contract for quote-path unclassified matcher/coherence/policy exceptions: sync and stream expose `E_INTERNAL` only (with parity), while any policy-owned typed internal exceptions remain internal implementation detail and transport mapping stays centralized at send-message boundaries.
  - `pr-05` currently plans sync quote-path failures after message preparation to be captured/finalized at the send-message boundary so the pre-created assistant message is not left `pending`; mapped user-visible error semantics are preserved while sync lifecycle behavior aligns with stream finalization discipline.
  - `pr-05` currently plans a dedicated sync pre-LLM quote-failure finalize contract (narrow helper/path at the send-message boundary) so quote-path failures before provider execution produce truthful persistent records: assistant `status='error'` with mapped external code + user-safe content, a pre-provider `message_llm` row with provider/model/prompt_version but `provider_request_id`/usage null and mapped `error_class`, no budget/BYOK/provider-execution side effects, and a distinct pre-LLM quote-context failure finalize event without duplicate policy anomaly logging.
  - `pr-05` currently plans to implement the sync pre-LLM quote-failure finalize path via a dedicated narrow helper/boundary in `send_message.py` (not broad `phase3_finalize(...)` reuse), keeping `phase3_finalize(...)` scoped to actual provider-execution finalize behavior and limiting any shared extraction to narrow private utilities rather than a generic finalize framework.
  - `pr-05` currently plans to catch/normalize pre-LLM quote-path failures at the outer sync `send_message(...)` boundary (narrow typed catches around `phase2_execute(...)`), finalize through the dedicated pre-LLM helper, and keep `phase2_execute(...)` focused on key resolution + quote/prompt/provider execution and provider-error normalization rather than quote-path transport/finalize mapping.
  - `pr-05` currently plans conditional/exactly-once semantics for the sync pre-LLM quote-failure finalize helper (aligned with sweeper/stream finalize posture): conditional assistant finalize on `status='pending'`, duplicate-safe `message_llm` insertion, non-clobbering race-lost handling with distinct observability, and preservation of the original mapped external quote-path error returned to the caller.
  - `pr-05` currently plans read-only quote/context rendering semantics for pending/invalidation PDF match metadata: deterministic in-memory enrichment attempts are allowed, but no hidden persistence/backfill of `plain_text_match_*` or `prefix/suffix` during quote rendering; explicit write/maintenance paths own persistence/backfill.
  - `pr-05` currently plans classified coherence validation for persisted PDF match metadata before using nearby-context offsets: persisted metadata is advisory, incoherent metadata is treated as untrusted and handled via runtime pending/unresolved + safe degradation (or fail-closed on unclassified validator exceptions), and quote rendering must not trust persisted `prefix/suffix` independently of validated offsets or safe in-memory matcher output.
  - `pr-05` currently plans to keep coherence validation in the PDF quote renderer path but route coherence-anomaly observability/logging/mapping through shared `pdf_quote_match_policy.py` helpers with a distinct canonical event schema (not `pdf_quote_match_anomaly`), preserving centralized helper ownership/no-double-logging and no-content logging posture.
  - `pr-05` currently plans a canonical coherence-anomaly policy contract in `pdf_quote_match_policy.py`: event `pdf_quote_context_coherence_anomaly`, policy-owned single-event logging, required/optional non-content fields, typed recoverable fallback outcome helper, typed fail-closed exception helper, and the same no-content redaction/no-double-logging posture as matcher anomalies.
  - `pr-05` currently plans a small typed recoverable coherence-anomaly taxonomy in `pdf_quote_match_policy.py` (`unsupported_match_version`, `status_offsets_inconsistent`, `offsets_out_of_range`, `offsets_outside_page_span`, `offset_substring_mismatch_exact`, `exact_status_inconsistent`) with exhaustive deterministic code->action mapping for recoverable quote-path fallback and fail-closed handling for unknown/unclassified cases.
- **non-goals**:
...
- **acceptance**:
  ...
  - `pr-07` L4 must define and test an explicit overlay redraw/reprojection trigger matrix (at minimum: page/text-layer render availability, viewer zoom/rotation changes, and highlight-data changes) rather than relying on implicit redraw behavior.
  - If any DOM text-walking remains in `pr-07` for PDF selection capture or overlay projection support, the relevant helpers must use a shared text-layer eligibility/filtering domain (no selection-vs-overlay text-node domain asymmetry).
