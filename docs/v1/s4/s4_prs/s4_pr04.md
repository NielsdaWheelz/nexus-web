...
## checklist
- [x] acceptance: endpoint `POST /libraries/{library_id}/invites`
- [x] acceptance: endpoint `GET /libraries/{library_id}/invites`
- [x] acceptance: endpoint `GET /libraries/invites`
- [x] acceptance: endpoint `POST /libraries/invites/{invite_id}/accept`
- [x] acceptance: endpoint `POST /libraries/invites/{invite_id}/decline`
- [x] acceptance: endpoint `DELETE /libraries/invites/{invite_id}`
- [x] acceptance: accept flow is transactional (`lock -> state check -> membership upsert -> invite update -> backfill-job upsert`)
- [x] acceptance: idempotent semantics for accept/decline/revoke match l2
- [x] acceptance: default-library invite target forbidden
- [x] acceptance: invitee lookup is user-id only with `404 E_USER_NOT_FOUND`
- [x] acceptance: matching next.js bff proxy routes for invite endpoints
- [x] acceptance: integration coverage for invite transitions + error masking
- [x] every traceability row has at least one planned test
- [x] every behavior-changing decision is captured in decision ledger
