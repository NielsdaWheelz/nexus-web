...
## checklist
- [x] `can_read_media` and `can_read_media_bulk` enforce s4 provenance semantics.
- [x] canonical `can_read_conversation` and `can_read_highlight` predicates are added.
- [x] conversation service has explicit `visible-read` and `owner-write` helpers; ambiguous helper removed.
- [x] highlight service has explicit `visible-read` and `author-write` helpers; ambiguous helper removed.
- [x] no public endpoint behavior/schema change is introduced in pr-02.
- [x] `python/nexus/services/search.py` is decoupled from deprecated helper naming with no behavior change.
- [x] default-library intrinsic write-through is applied in `upload`, `libraries`, and ingest dedupe attach paths.
- [x] strict revocation is covered by helper/predicate-level tests, including share-row revocation and membership revocation.
- [x] conversation visible-read helper tests cover owner, public, and library-shared branches.
- [x] fixture helpers in `test_search.py` and `test_send_message.py` are updated for intrinsic-aware default-library seeding.
- [x] new/updated tests in `test_permissions.py`, `test_visibility_helpers.py`, `test_libraries.py`, `test_upload.py`, and `test_from_url.py` pass.
- [x] compatibility regression checks for conversations/highlights/search/send-message remain green.
