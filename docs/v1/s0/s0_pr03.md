...
- **All domain logic lives in `app/services/libraries.py`.**
...
from app.schemas.libraries import CreateLibraryRequest, LibraryOut
...
A test **MUST** fail if any `app/api/*.py` file contains:

**Forbidden imports:**
- Any import from `sqlalchemy` **except** `from sqlalchemy.orm import Session`
- Any import from `app.db` **except** `from app.db.session import get_db`
...
- `from app.db.session import get_db` (dependency only)
- Pydantic schemas from `app.schemas.*`
- Service modules from `app.services.*`
- Response helper `success()` from `app.responses`
- `ApiError` from `app.errors`
...
**Note:** `author` is NOT included. The s0_spec.md media schema does not have an `author` column. If PR-01 migration includes `author`, it must be excluded from this output schema for S0 consistency.
...
All functions live in `app/services/libraries.py`.
...
All routes live in `app/api/libraries.py`.
...
A test **MUST** fail if:
- Any `app/api/*.py` violates the import rules in §4.2
