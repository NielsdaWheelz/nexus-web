...
1. **Preferred:** Update `s0_pr01.md` migration spec to use these values. PR-04 adds no corrective migration.
2. **Fallback:** If `main` already has wrong constraint merged, PR-04 includes a corrective migration:
   - Drops the existing CHECK constraint
   - Adds the correct CHECK constraint

Do NOT knowingly merge PR-01 with incorrect values and "fix later." Fix at source.
...
- New tables
- New columns
- Schema changes beyond the processing_status constraint fix (if needed)
...
Registered in `app/main.py` (no prefix).
...
- `app/services/media.py` exists and is used by routes
