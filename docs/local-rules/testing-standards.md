...
kernel-reported available memory to reach 3,584 MiB: Linux `MemAvailable`, or
...
After each browser capability, the controller retires every exact run-owned
application and protocol process before admitting another heavy capability.
Migrated databases, the object-store bucket, protocol peer state, standalone
build, and browser data epoch remain reusable within that invocation. Process
restart appends logs and retains peer audit state; failed retirement fails the
capability and preserves outstanding recovery ownership.
