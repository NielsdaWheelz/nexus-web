...
- Render EPUB images, CSS-referenced assets, SVGs, fonts, and other local
  resources through controlled app routes.
...
- EPUBs that previously missed images or local styling render stored local
  resources through app-controlled asset URLs.
...
EPUB ingest task
  ...
  writes local assets to storage
...
Asset service
  stores manifest resources under deterministic keys
  records resource metadata in DB
  serves resources through BFF-backed routes
...
- Sanitized HTML, canonical text, navigation rows, asset rows, content chunks,
  generated tsvectors, and embeddings are derived read models.
...
- Every resource needed for reader rendering is stored or the ingest fails.
...
- Do not use regex for HTML, XML, CSS, or URL reference rewriting.
...
- Strip or neutralize event handlers, forms, inline scripts, remote script
  references, and unsafe URLs during sanitization.

## Asset Rules

- Store all local resources required for rendering:
  - raster images
  - SVG images
  - CSS
  - fonts
  - audio and video resources when referenced by content
  - poster images
  - cover image
...
`GET /media/{id}/assets/{asset_key}` returns one stored EPUB asset.

Rules:

- Permission checks match media read permission.
- Only stored, media-owned assets can be served.
- Missing assets return one explicit error code.
...
- Ingest does not mark media readable until required DB rows and required stored
  assets exist.
- An asset that is referenced by rendered content but not stored is an ingest
  failure.
...
- `python/nexus/services/epub_ingest.py`
  - Replace with orchestration over parser, resource, asset, and persistence
    helpers.
- `python/nexus/services/epub_publication.py`
  - New EPUB package parser and publication graph owner.
- `python/nexus/services/epub_assets.py`
  - New asset classification, storage, and lookup service.
- `python/nexus/services/epub_read.py`
  - Read persisted reader models only.
- `python/nexus/services/sanitize_html.py`
  - Keep sanitization centralized. Add EPUB-specific allowed-resource URL
    handling only through typed inputs.
...
- `python/nexus/api/routes/media.py`
  - Add asset route and keep route handlers transport-shaped.
...
- `apps/web/src/app/(authenticated)/media/[id]/EpubContentPane.tsx`
  - Keep rendering through `HtmlRenderer`.
- `apps/web/src/components/HtmlRenderer.tsx`
  - Remains the only direct sanitized HTML insertion point.
...
- CSS URL rewriting resolves paths from the CSS resource base path.
- `srcset` rewriting preserves descriptors.
...
- Ingest persists all required render assets.
- Ingest fails when a referenced local render asset cannot be stored.
...
- Asset route enforces media read permission and serves stored assets.
...
- A real EPUB opens through the same media page and renders images.
...
- Every rendered local resource URL resolves through `/api/media/{id}/assets`.
...
- EPUB ingest cannot mark media readable when required assets are missing.
...
- All render-required local assets are stored.
