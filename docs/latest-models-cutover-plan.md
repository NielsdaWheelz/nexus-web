...
status: implementation plan; no code or live qualification yet
...
[block reordering](tickets/anthropic-continuation-loses-native-block-order.md)
and [lost earlier turns](tickets/provider-successor-drops-earlier-tool-turns.md).
...
repair [codex mcp admission](tickets/restored-chat-codex-dispatch-fails.md)
...
stage and generation identity through [host failures](tickets/codex-host-discards-original-runtime-failure.md).
...
preserving unsent text in each browser/webview. start at the approved chat seed.
...
checks and ordinary use. native app-server is vendor-experimental, so exact
