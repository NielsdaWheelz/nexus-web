...
1. **The Argument** — `stance_scan` job after `media_unit_build`: new claims
   ...
   `synapse.py`.
2. **Take a Side** — two-key chord in the reader minting a _user-origin_ stance
...
3. **The Reckoning** — weekly disputation spread: two cited quotes facing each
   other across a center rule; verbs _side with · hold both · not a
   contradiction_. Every ruling writes a stance edge or suppression.
4. **Ledger of Positions** — coin a belief as one signed sentence; reading files
   in behind or against it in two columns. Standing (_settled / contested /
   eroding_) derived from evidence balance, never set by hand.
5. **The Canon** (epoch) — the LI artifact/revision engine pointed at the whole
   ...
   settled belief (_concede / rebut / dismiss_).
...
- **The Expedition** — commissioned audit of what the corpus _doesn't_ cover on
