...
**Type:** hard cutover · mobile presentation · 80/20 slice
...
- backend, API, database, persistence, Android-native, iOS-native, or release
  changes;
...
| D — contract and closeout | this spec, `docs/cutovers/nexus-intent-router-hard-cutover.md`, `docs/modules/app-navigation.md` | Superseded prose removed, residue audit clean, evidence classified |
...
Risk score: **medium**. This is browser interaction/layout with no persistence,
irreversibility, provider, or native change. Real Chromium is the primary
boundary; physical WebView review is supplemental platform evidence.
...
5. **Integration:** run `./scripts/test confidence`, then `./scripts/test pr`
   ...
   no new journey and do not duplicate its workspace contract. The current
   registry already owns Nexus source globs; `testdata/proofs.json` does not
   change.
6. **Physical:** run `./scripts/test android-visual --sha <exact-commit-sha>
   --path / --device primary`, then perform the operator checks below.

Do not run `full`, `nightly`, `release`, hosted-provider, database, or native
instrumentation solely for this cutover unless the typed controller selects a
newly affected owner.
...
or USB-attested evidence and must never be reported as such. This cutover owns
no native behavior, so those stronger lanes are not required.
...
11. No backend, persisted, wire, provider, database, native, or release contract
    changes.
