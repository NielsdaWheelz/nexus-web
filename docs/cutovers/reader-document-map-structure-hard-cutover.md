...
status: specified; implementation not started. approved direction:
...
- explicit publisher navigation plus content headings determine structure.
  missing structure remains missing; no inferred chapter or guessed offset.
...
| navigation section | `{section_id, label, parent_section_id: Presence<id>, target: text point, extent: Presence<text range>, source: Publisher \| Heading \| Both}` |
...
3. semantic parent is the nearest source-supported containing section. use
   ...
   contradictory hierarchy as absent extent, not guessed containment.
...
retain `epub_toc_nodes` for publisher order/hierarchy. adapt
...
when extents exist. validate cross-row bounds/containment at the owning write
boundary; local shape constraints belong in the database. web headings retain
their existing storage owner and use the same semantic range builder.
...
| `GET /media/{id}/document-map` | embeds the same navigation and generation; retains marker ids, exact-source-derived `position`, activation item refs, and decoration fields; adds `end_position: Presence<number>` for exact ranges |
...
find activation outputs to fragment loading; persisted fragment-offset passage
...
2. rebuild metadata from stored sanitized html, canonical text, source-fragment
   ...
   target. a record lacking that exact address aborts the migration.
3. before removing old nav rows, convert every epub resume target using its
   existing section→fragment relation. preserve locations/text, convert anchor
...
runner bypass. no tests run for this spec-only change.
