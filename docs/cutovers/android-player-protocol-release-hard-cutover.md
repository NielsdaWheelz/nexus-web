...
pump. A parked intent is released whenever its receipt stops being current
(acknowledged, superseded, or cleared); it is never left addressed to a receipt
that no longer exists. Independent `Dismiss`, `PodcastSettings`, and
`ListeningProjection` operations are bounded latest-wins keys; `Dismiss` is its
own key because native exempts it from the pending-receipt barrier. No barrier
path renders the generic Retry action.

The web pump is one pure state machine, `lib/player/nativeOperationPump.ts`.
The runtime executes its `Run` effects and derives the shell's visible
transport failure from its frozen operations, so no other dispatch can clear a
frozen intent's Retry. One exhaustive classifier in `androidPlayerRuntime.tsx`
owns every bridge failure: version skew, retryable transport (bridge absent,
timeout, unconfirmed reconciliation, `PlayerUnavailable`, `AccountMismatch`),
the barrier, cancellation by a closed client, and defects. A replacement
controller drops a frozen session intent as ambiguous against its authoritative
snapshot and resumes frozen bounded operations that still apply.
...
`PlayerCommandParseResult.Rejected` carries its `PlayerRejectionCode` and its
exact v2 wire `reply`. `NexusPlayerBridge` and `NexusPlaybackService` post that
reply verbatim; they hold no code to substitute. This removes the current
duplicate mapping without introducing another layer. Native strict JSON
accessors read through `opt`, so Android's checked `JSONException` can never
escape the classifier's `RuntimeException` catches on a device.
...
|---|---|---|
| A — Contract | This spec; `.gitattributes`; `testdata/android/player-protocol.json`; `testdata/{manifest,proofs}.json`; `testdata/faults/{manifest.json,android-player-protocol-skew-bypass.patch}`; `python/nexus/release_artifact.py` (`AndroidPlayerProtocolIdentity`); `python/nexus_test_control/{model,policy}.py`; `python/tests/kernel/{test_backend_artifact.py,nexus_test_control/{test_model,test_policy,test_selection}.py}` | Freeze byte-exact exhaustive v2 oracle, typed priority risk/ownership digest, and fault before implementation. |
| B — Web | `apps/web/{next.config.ts,vitest.config.ts,package.json,androidPlayerProtocolCorpus.ts,e2e/journeys/auth-session.journey.spec.ts}` (the journey for the `/version` shape only); `app/android/{page.tsx,AndroidPage.browser.test.tsx}`; `lib/{androidReleaseLinks.ts,androidReleaseLinks.unit.test.ts}`; `app/version/{route.ts,route.unit.test.ts}`; player `androidPlayer{Protocol,Client,Runtime}.ts(x)`, `nativeOperationPump{.ts,.unit.test.ts}`, `browserPlayerRuntime.tsx`, `playerRuntime.tsx`, `playerChromeModel.ts`; `lib/actions/resourceActionRuntime.tsx`; `GlobalPlayerSurfaces{.tsx,.module.css,.browser.test.tsx}` | Strict identity/body decoder, typed state, exhaustive consumers, test-time digest injection, Update link ownership, UI, and web proofs. Consumes A read-only. Paths after the config entries are below `apps/web/src`. |
| C — Android | `apps/android/app/build.gradle.kts`; `src/main/AndroidManifest.xml`; playback `PlayerProtocol.kt`, `NexusPlayerBridge.kt`, `NexusPlaybackService.kt`; webkit `OwnedOriginWebMessage.kt` (strict accessors); `PlayerProtocolTest.kt`, `NexusPlaybackServiceContractTest.kt`, `webkit/StrictJsonAccessorsTest.kt` | Strict v2 native wire, one rejection classifier, embedded identity, native proof. Paths after Gradle are below `apps/android/app`. Consumes A read-only. |
| D — Release | `.github/workflows/backend-images.yml`; `python/nexus_test_control/runner.py`; `python/tests/kernel/nexus_test_control/test_runner.py`; `deploy/hetzner/{deploy.sh,fetch-release-bundle.sh,release.py}`; `python/tests/kernel/{test_android_player_protocol_release_gate,test_release_bundle_fetch,test_successor_release_contract,test_production_delivery_contract,test_production_release,test_production_deploy_behavior,test_oracle_host_release}.py`; `python/tests/testkit/{host_release,host_oracle_reconcile,production_deploy,release_bundle}.py` | Signed manifest v2; local clean-checkout compatibility gate for a new candidate before bundle, Vercel, resource-limit, or host mutation; provider-independent durable resume/settlement; and byte-exact corpus inclusion in the closed immutable bundle for remote post-promotion/resume proof. Reuse the existing Android release workflow and asset upload unchanged. Consumes A read-only. |
...
| Native wire ingress/egress | `PlayerProtocolTest.kt` consumes the same corpus: exact v2 variants round-trip; every sealed inventory is derived from the production types by reflection; any noncurrent identity returns `ProtocolMismatch` without body parsing, and its carried `reply` is the exact v2 wire; malformed v2 identity is `InvalidRequest`. Consumer preservation is structural: `Rejected.reply` is the only thing the bridge and service post. `webkit/StrictJsonAccessorsTest.kt` proves an absent key surfaces as the owned exception type, not `JSONException`. |
| Player presentation | Real Chromium `GlobalPlayerSurfaces.browser.test.tsx`: install a narrow fake only at external `window.nexusPlayer`, mount the public providers and the real surfaces in every scenario, and prove Update copy/link/focus are visible while Retry is absent. One transient-unavailability case preserves Retry; one matching-identity malformed reply reaches the defect boundary with neither Update nor Retry. `nativeOperationPump.unit.test.ts` proves the pump invariants (derived Retry, barrier release, FIFO, latest-wins). The pure `lib/androidReleaseLinks.unit.test.ts` owns the exact stable URL model without file I/O or React; `app/android/AndroidPage.browser.test.tsx` owns its rendered projection. |
...
| Deploy coordination | `test_android_player_protocol_release_gate.py` drives the production-release kernel harness's loopback GitHub boundary: the exact stable Android release behind `releases/latest` permits a new candidate; a non-Android/draft/prerelease/missing/malformed/skewed pointer or manifest stops before mutation with its named reason, on a fresh host and on an installed bundle; resume, settlement, and current verification make no GitHub contact (`gh` or `git fetch`). The immutable release suites keep their own owner. Do not contact GitHub in PR proof. |
