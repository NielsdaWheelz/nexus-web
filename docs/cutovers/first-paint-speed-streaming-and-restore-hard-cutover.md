...
- **C7 — Best-effort everywhere.** Any bootstrap fetch may fail/timeout without affecting correctness — the client owns retry/refetch; a slow backend degrades latency, never correctness. *(Superseded 2026-07-16: the reader-profile read is now a required exception to C7 — it is required for the interactive shell to reveal, on the normal 30 s deadline; failure/timeout replaces the shell with an accessible error and Retry, never a fabricated default. Session restore and pane resource seeds remain best-effort as described here. See `docs/cutovers/reader-profile-persistence-hard-cutover.md`.)*
