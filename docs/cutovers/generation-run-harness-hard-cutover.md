...
- Oracle readings: user-library passages and resolved public-domain `oracle_passage_anchor` passages render citation chips through the shared `CitationOut` read model; unresolved/span-less targets keep their typography. A "Consult the archive" (chat) action opens a conversation referencing the reading.
...
**Failure floor.** `run_kit.mark_terminal` grows `error_code: str | None = None, error_detail: str | None = None` and stamps them on the parent via its existing isinstance dispatch (oracle also gets `failed_at` set there, satisfying `ck_oracle_readings_failed_has_error`). `error_detail` = sanitized `f"{type(exc).__name__}: {exc}"[:1000]` + provider request id when known — operator-facing, never rendered. The fail-after-worker-exception trio becomes one `fail_run_after_worker_exception(stream_loader, …)` helper parameterized by terminal-predicate + failure write. Superseded for durable chat by `chat-durable-agent-step-journal-hard-cutover.md`: defects remain nonterminal through queue retry, and the chat dead-letter hook writes no assistant message, run status, or stream event; it records safe diagnostics and may requeue a prior cancellation request. `LLM_ERROR_CODE_TO_API_ERROR_CODE` gains `QUOTA_EXCEEDED → E_LLM_QUOTA_EXCEEDED`. The only backend code→user-copy map left is chat's `ERROR_CODE_TO_MESSAGE` (it writes assistant content for modeled terminal failures); oracle's `_oracle_failure_message` + its 7 constants + the read-time event rewrite (`_oracle_event_out`, oracle.py:965-970) are deleted — the FE already owns oracle failure copy (`oracleFailureFeedback`).
...
4. **Oracle passages join the contract (read-model only, no storage change):** user-media evidence and resolved public-domain `oracle_passage_anchor` targets mint `CitationOut` via the shared citation read model; ordinal = phase order (descent 1, ordeal 2, ascent 3). Exposed as `OracleReadingPassageOut.citation: CitationOut | None` and on the `passage` SSE payload. Page-owned chunks, missing spans, and unresolved anchors ⇒ `citation=None`. FE renders a `ReaderCitation` chip beside `locator_label` for passages with a citation; folio typography untouched.
...
- **S2 — ledger + error floor.** Migration 0145; `run_kit.mark_terminal(error_code, error_detail)`; `llm_ledger` (`llm_calls` writer + `observed_llm_call`); chat modeled terminal folds write parent detail and provider calls write ledger rows. Superseded for durable chat: defect/dead-letter paths no longer terminalize or stamp the run parent. Oracle/LI/unit/enrich terminal writes adopt the floor (no behavioral change yet beyond persistence).
...
- **AC-10 (citations server-built).** `GET …/messages` returns `citations`; the FE builds no CitationOut from retrievals/blocks (`messageToCitationOuts` gone); web_search rows flow through `insert_retrieval_row`; oracle user-media passages and resolved public-domain anchors render working citation chips; `oracle_reading:{id}` attaches and reads in chat; a chat run over an LI artifact stamps the consumed `revision_id` in `included_context_refs`.
...
- **Opaque means opaque:** `provider_items`/`provider_metadata` are never parsed,
  transformed, or logged with content by Nexus. Superseded for durable chat only
  by `chat-durable-agent-step-journal-hard-cutover.md`: the typed continuation
  artifact is normalized into the private job journal so a completed step can be
  resumed exactly. Other generation owners retain their declared policy.
...
13. **Oracle citations are read-model only** — folio storage stays phase/passage-owned; user-media passages and resolved public-domain anchors mint `CitationOut`, while unresolved/span-less targets stay typographic; ordinal = phase order.
