...
the delivery now retains all 685 cited and local verification runs in
`testdata/evidence/bounded-workspace-receipts.json`: 417 pass, 264 fail and four
not-run results, with original summary/context hashes and artifact identities.
all 1,370 metadata files reconstruct byte-for-byte. no historical verdict was
changed. the index is 3,333,268 bytes; it keeps metadata, not raw artifact bodies.
the repository policy checks coverage and retained hashes. its ordinary proof
passed within `b45be17460a6b9cc`; the missing-reference BASE replay remains
required before closing this ticket. the enclosing run failed later in another
controller fixture.
