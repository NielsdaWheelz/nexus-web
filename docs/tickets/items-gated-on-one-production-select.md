...
1. retired dossier failure codes (ART-04 / WLR-03).
   ...
   (`_tag_historical_failures`); `engine.py:4043-4046` refuses to emit the event
...
2. system-role chat messages (CHAT-03). `SELECT count(*) FROM messages WHERE
   ...
   (`chat_run_candidates.py:240,265`, `chat_run_message_prep.py:61,84`) write
   ...
   (`messageWire.ts:329-335`) spreads the raw object without validating `role`,
...
4. pgvector version (ATLAS-PY-FALLBACK). `SELECT extversion FROM pg_extension
   WHERE extname = 'vector'`. `services/atlas_projection.py:94-147` catches
