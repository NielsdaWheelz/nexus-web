...
`python/nexus/tasks/llm_task.py:30-69` runs each chat job's async handler on a
private event loop (`loop.run_until_complete`). on that loop,
`python/nexus/services/chat_runs.py:1010-1248` executes synchronous sqlalchemy
work: `is_cancel_requested(db, ...)` (line 1073), `lock_chat_run_for_update`,
event-store appends from the text coalescer flush, and `db.commit()` (line
1248). the same loop drives the provider stream, the 33 ms coalescer flush
timer (`CHAT_TEXT_FLUSH_INTERVAL_MS`, line 226), and the cancel watcher
(`_watch_chat_run_cancel`, lines 992-1008), which opens a second session and
reads on the loop every 0.25 s.
...
ownership; keep the MCP listener on its separate loop
(`python/nexus/db/async_session.py`).
