...
revision: str = "0221"
down_revision: str | Sequence[str] | None = "0220"
...
def _required_environment(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"0221 backfill requires {name}")
    return value
...
def _required_timeout(name: str, default: float) -> float:
    ...
    try:
        ...
        raise RuntimeError(f"0221 backfill: {name} must be a number") from exc
    if value <= 0:
        raise RuntimeError(f"0221 backfill: {name} must be positive")
    return value
...
def _measure_source(
    *, client, bucket: str, media_id: object, path: str, kind: str, size: int
) -> str:
    expected_signature = _SIGNATURES.get(kind)
    if expected_signature is None:
        raise RuntimeError(f"0221 backfill: media_file {media_id} has unsupported kind {kind!r}")
    try:
        metadata = client.head_object(Bucket=bucket, Key=path)
        stored_size = int(metadata.get("ContentLength") or 0)
        if stored_size != size:
            raise RuntimeError(
                f"0221 backfill: media_file {media_id} size changed "
        ...
        raise RuntimeError(
            f"0221 backfill: failed to read media_file {media_id} object {path!r}"
        ) from exc
    if measured_size != size:
        raise RuntimeError(
            f"0221 backfill: media_file {media_id} stream size changed "
            f"(database={size}, storage={measured_size})"
        )
    if bytes(prefix) != expected_signature:
        raise RuntimeError(
            f"0221 backfill: media_file {media_id} object {path!r} has an invalid {kind} signature"
        )
...
def _backfill_source_digests() -> None:
    ...
    for row in rows:
        ...
        bind.execute(
            sa.text("UPDATE media_file SET source_sha256 = :digest WHERE media_id = :media_id"),
...
def _assert_no_active_source_publication_defects() -> None:
    ...
    if defects:
        raise RuntimeError(
            "0221 preflight: active source publication defects require exact operator cleanup: "
...
def upgrade() -> None:
    ...
    # changed, the revision remains at 0220 and a rerun skips completed rows.
    ...
    if invalid:
        raise RuntimeError(
            "0221 preflight: media_file source_sha256 backfill is incomplete: "
...
def downgrade() -> None:
    raise NotImplementedError("0221 is an irreversible document-import hard cutover")
