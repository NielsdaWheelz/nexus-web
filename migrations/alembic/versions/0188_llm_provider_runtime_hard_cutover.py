...
NUMBERING NOTE: rechained onto 0187 at merge (the persistence half was authored
against 0183, but main advanced first with 0184 universal-link-authoring,
0185 remove-user-pinned-objects, 0186 podcast-listening `last_engaged_at`, and
0187 library reading-time word counts — all orthogonal to the
llm_calls/chat_runs/models/user_api_keys tables this migration transforms).
...
revision: str = "0188"
down_revision: str | Sequence[str] | None = "0187"
...
def _fail(phase: str, message: str) -> None:
    raise RuntimeError(f"0188 {phase}: {message}")


def _report(message: str) -> None:
    print(f"0188: {message}")
...
def downgrade() -> None:
    raise NotImplementedError(
        "0188 is a hard cutover migration and has no downgrade path: it drops"
