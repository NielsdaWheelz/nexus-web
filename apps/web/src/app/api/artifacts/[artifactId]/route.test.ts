...
describe("GET /api/artifacts/[artifactId]", () => {
  ...
  it("proxies artifact detail requests to the canonical artifact backend path", async () => {
    const fetchMock = vi.fn<typeof fetch>(
      async () => Response.json({ data: { id: "artifact-1" } })
    );
    vi.stubGlobal("fetch", fetchMock);

    const response = await GET(
      new Request("http://localhost:3000/api/artifacts/artifact-1", {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://fastapi.test/artifacts/artifact-1",
