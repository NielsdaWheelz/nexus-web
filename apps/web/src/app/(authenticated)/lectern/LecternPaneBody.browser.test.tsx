    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/resource-items/action-snapshots/resolve") {
        ...
        return Response.json({
          data: { snapshots: refs.map(resolveSnapshot) },
        });
      }
...
describe("Lectern pane refinement", () => {
  ...
  it("offers reorder only in unfiltered Custom order while remove stays available in every view", async () => {
    ...
    // Filtered view: the reorder control disappears, remove stays available.
    await userEvent.type(filterInput(), "rumor");
    await waitFor(() => expect(renderedTitles()).toEqual([RUMOR]));
    expect(
      screen.queryByRole("button", { name: `Reorder ${RUMOR}` }),
    ).toBeNull();
    await openRowMenu(RUMOR);
    expect(
      screen.getByRole("menuitemcheckbox", { name: "Remove from Lectern" }),
    ...
    // Sorted view: the reorder control disappears, remove stays available.
    await userEvent.clear(filterInput());
    await waitFor(() => expect(renderedTitles()).toHaveLength(3));
