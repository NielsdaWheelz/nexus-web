...
describe("ConnectionsSurface", () => {
  ...
  it("shows API errors instead of an empty connections state", async () => {
    ...
    render(
      <ConnectionsSurface
        objectRef={{ objectType: "note_block", objectId: BLOCK_A }}
      />,
    );
  ...
  it("aborts stale reads and renders the latest object's connection", async () => {
    ...
    rerender(
      <ConnectionsSurface
        objectRef={{ objectType: "note_block", objectId: BLOCK_B }}
      />,
    );
  ...
  it("retries only the connection after an accepted upload edge failure", async () => {
    const user = userEvent.setup();
    let loadedAttachment = false;
    ...
    expect(
      await screen.findByText(
        "paper.pdf was saved and still needs its connection.",
      ),
    ).toBeInTheDocument();
    const toggle = screen.getByRole("button", { name: "＋ Connect" });
  ...
  it("retains the accepted identity when the edge and post-init ingest both fail", async () => {
    const user = userEvent.setup();
            ...
            return Response.json(
              {
                error: {
                  code: "E_EDGE_WRITE_FAILED",
                  message: "Connection write failed",
                  request_id: "req-edge-double-failure",
                },
              },
              { status: 503 },
            );
  ...
  it("creates one durable edge before surfacing a post-init upload defect", async () => {
    const user = userEvent.setup();
    const consoleError = vi
      .spyOn(console, "error")
      .mockImplementation(() => {});
        ...
        if (path === "/api/media/upload/init") {
          ...
          return Response.json({
            data: {
              ...
              processing_status: "pending",
              ingest_enqueued: false,
              upload_url: "https://uploads.example/post-init-defect.pdf",
              expires_at: "2026-01-01T00:00:00Z",
            },
          });
        }
        if (path === "https://uploads.example/post-init-defect.pdf") {
          return new Response(null, { status: 200 });
        }
        if (path === `/api/media/${MEDIA_ID}/ingest`) {
          return Response.json(
    ...
    try {
      ...
      await user.upload(
        screen.getByLabelText("Attach files"),
        new File(["%PDF-1.7"], "post-init-defect.pdf", {
          type: "application/pdf",
        }),
      );
  ...
  it("surfaces same-system upload defects instead of rendering attachment feedback", async () => {
    const user = userEvent.setup();
    const consoleError = vi
      .spyOn(console, "error")
      .mockImplementation(() => {});
    ...
    try {
      ...
      expect(
        await screen.findByText("Connections need attention"),
      ).toBeInTheDocument();
      expect(
  ...
  it("short-circuits without polling when the scan request reports idle", async () => {
    ...
    render(
      <ConnectionsSurface
        objectRef={{ objectType: "media", objectId: MEDIA_ID }}
      />,
    );
    await waitFor(() => expect(connectionReads(requests)).toHaveLength(1));
    connectionReads(requests)[0].resolve(connectionResponse([]));
