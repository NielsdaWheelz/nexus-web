...
describe("add content staging", () => {
  ...
  it("starts a Web Search handoff with the URL draft without staging it", () => {
    const state = createAddSessionState({
      seed: {
        ...contentSeed,
        initialUrlDraft: "https://example.com/from-web",
      },
      sessionId: "session-web",
    });

    expect(state.urlInput.text).toBe("https://example.com/from-web");
    expect(state.items).toEqual([]);
    expect(state.mutation).toEqual({ kind: "Idle" });
  });
