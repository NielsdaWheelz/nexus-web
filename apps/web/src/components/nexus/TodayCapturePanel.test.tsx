 ...
 * `__tests__/components/AddContentTray.test.tsx` after the Nexus cutover
...
function renderTodayCapturePanel(): {
  onOpen: ReturnType<typeof vi.fn<(target: NexusTarget) => void>>;
  onBack: ReturnType<typeof vi.fn<() => void>>;
} {
  const onOpen = vi.fn<(target: NexusTarget) => void>();
...
describe("TodayCapturePanel", () => {
  ...
  it("flushes the typed draft and routes to today through the shell owner", async () => {
    ...
    expect(onOpen).toHaveBeenCalledWith({ kind: "OpenToday" });
