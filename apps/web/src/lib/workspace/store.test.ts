...
describe("workspace reducer", () => {
  ...
  it("reuses an open resource pane and updates its href", () => {
    ...
    const next = workspaceReducer(withChat, {
      type: "open_pane",
      panes: [makePane(duplicateId, "/conversations/conversation-1?run=run-new")],
      afterPaneId: null,
      activate: true,
    });
    ...
    expect(next.panes.find((pane) => pane.id === chatId)?.href).toBe(
      "/conversations/conversation-1?run=run-new",
    );
