...
export type SessionVerification =
  ...
  | { kind: "SignInRequired"; cookieNames: readonly string[] };

async function clearAuthCookies(cookieNames: readonly string[]): Promise<void> {
 ...
 * - `refreshable`: return a refresh-required outcome.
 * - `ended` / `anonymous`: return a sign-in-required outcome with the exact
 *   cookie names a response owner must clear.
 *
 * Memoized per request so page and route projections share one verification.
 * `verifySession()` owns Server Component redirects; route handlers project
 * the same outcome onto method-safe responses they own.
 */
    ...
    switch (session.state) {
      case "active": {
        ...
        try {
          ...
          console.error("auth_session_verify_failed", {
        ...
        return { kind: "SignInRequired", cookieNames: session.cookieNames };
      }
      case "refreshable":
...
export async function verifySession(): Promise<Viewer> {
  ...
  switch (verification.kind) {
    ...
    case "SignInRequired":
      await clearAuthCookies(verification.cookieNames);
