...
describe("conversation provenance", () => {
  it("builds a source-claim lineage model from message document blocks", () => {
    ...
    expect(countProvenanceSignals([provenanceMessage])).toBe(2);
  ...
  it("formats a portable audit brief for the provenance graph", () => {
    ...
    expect(brief).toContain("1 assistant turn, 1 source, 1 claim.");
    ...
    expect(brief).toContain("- Deep Work: 1/1 retrieved, 1 claim links");
  });
