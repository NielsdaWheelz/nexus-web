...
/**
 * BrowserPlayerRuntimeProvider — the non-Android audio runtime (spec
...
import {
  OUTPUT_EFFECTS_DEFAULTS,
  COMPRESSOR_DEFAULTS,
  VOLUME_BOOST_GAIN_BY_LEVEL,
  areOutputEffectsActive,
  normalizeVolumeBoostLevel,
  readOutputEffectsFromStorage,
  writeOutputEffectsToStorage,
  type OutputEffectsState,
...
import {
  ...
  type PlayerSessionCapability,
  type PlayerSettingsCapability,
  type PlayerTimelineCapability,
  type PreviewAudioPosition,
  type PreviewAudioSession,
...
function canCaptureOutputEffectsSource(sourceUrl: string): boolean {
  try {
    return (
      new URL(sourceUrl, window.location.href).origin ===
...
export function BrowserPlayerRuntimeProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const [outputEffects, setOutputEffectsState] = useState<OutputEffectsState>(
    OUTPUT_EFFECTS_DEFAULTS,
  );
  ...
  const outputEffectsRef = useRef<OutputEffectsState>(OUTPUT_EFFECTS_DEFAULTS);
  outputEffectsRef.current = outputEffects;
  const outputEffectsAvailableRef = useRef(true);
  outputEffectsAvailableRef.current = outputEffectsAvailable;
  ...
  const resetAudioGraphNodes = useCallback(() => {
    ...
    monoLeftGainNodeRef.current?.disconnect();
    monoRightGainNodeRef.current?.disconnect();
  ...
  const applyOwnedPlaybackRate = useCallback(
    (target: number) => {
      ...
      ownedRateExpectationRef.current = { element: audio, target };
  ...
  const applyCurrentPlaybackRate = useCallback(() => {
    applyOwnedPlaybackRate(basePlaybackRateRef.current);
  }, [applyOwnedPlaybackRate]);

  const configureOutputEffectsGraph = useCallback(() => {
    ...
    const effects = outputEffectsRef.current;
    let tail: AudioNode = source;
    if (effects.volumeBoost !== "off") {
      const gain = gainNodeRef.current ?? context.createGain();
      const compressor =
        compressorNodeRef.current ?? context.createDynamicsCompressor();
      gainNodeRef.current = gain;
      compressorNodeRef.current = compressor;
      gain.gain.value = VOLUME_BOOST_GAIN_BY_LEVEL[effects.volumeBoost];
      compressor.threshold.value = COMPRESSOR_DEFAULTS.threshold;
      compressor.knee.value = COMPRESSOR_DEFAULTS.knee;
      compressor.ratio.value = COMPRESSOR_DEFAULTS.ratio;
      compressor.attack.value = COMPRESSOR_DEFAULTS.attack;
      compressor.release.value = COMPRESSOR_DEFAULTS.release;
      tail.connect(gain);
    ...
    const left = monoLeftGainNodeRef.current ?? context.createGain();
    const right = monoRightGainNodeRef.current ?? context.createGain();
    const merger = mergerNodeRef.current ?? context.createChannelMerger(2);
    splitterNodeRef.current = splitter;
    monoLeftGainNodeRef.current = left;
    monoRightGainNodeRef.current = right;
  ...
  const markOutputEffectsUnavailable = useCallback((rotateElement: boolean) => {
    if (rotateElement) audioElementNeedsRotationRef.current = true;
    setOutputEffectsAvailable(false);
    outputEffectsAvailableRef.current = false;
  }, []);

  const ensureOutputEffectsGraph = useCallback((): AudioContext | null => {
    const audio = audioElementRef.current;
    if (!audio) return null;
    const sourceUrl =
      audio.currentSrc ||
      audio.getAttribute("src") ||
      pendingStartRef.current?.sourceUrl;
    if (
      sourceUrl === undefined ||
      !canCaptureOutputEffectsSource(sourceUrl)
    ) {
      markOutputEffectsUnavailable(false);
      return null;
    }
    ...
    if (context?.state === "closed") {
      markOutputEffectsUnavailable(true);
      return null;
    }
    if (!context) {
    ...
    setOutputEffectsAvailable(true);
    outputEffectsAvailableRef.current = true;
    configureOutputEffectsGraph();
    return context;
  }, [
    configureOutputEffectsGraph,
    markOutputEffectsUnavailable,
    resetAudioGraphNodes,
  ]);

  const resumeOutputEffectsGraphIfRequired =
    useCallback((): AudioContext | null => {
      if (
        audioContextRef.current === null &&
        !areOutputEffectsActive(outputEffectsRef.current)
      ) {
        return null;
      }
      const context = ensureOutputEffectsGraph();
      if (context?.state === "suspended") {
        void context.resume().catch(() => {});
      }
      return context;
    }, [ensureOutputEffectsGraph]);
  ...
  const prepareAudioElementForSource = useCallback(
    (sourceUrl: string): void => {
      const captureAllowed = canCaptureOutputEffectsSource(sourceUrl);
      if (!captureAllowed && sourceNodeRef.current !== null) {
        audioElementNeedsRotationRef.current = true;
      }
      if (
        captureAllowed &&
  ...
  const applyTransition = useCallback(
    (requestedTransition: SessionTransition) => {
      ...
      if (transition.effect.kind === "StartSession") {
        ...
        const sourceUrl = descriptor.activation.streamUrl;
        pendingStartRef.current = {
          sourceUrl,
          startSeconds: startPositionMs / 1000,
        };
        prepareAudioElementForSource(sourceUrl);
  ...
  const resume = useCallback(() => {
    const audio = audioElementRef.current;
    if (!audio) return;
    resumeOutputEffectsGraphIfRequired();
  ...
  const playPreviewAudio = useCallback(
    (descriptor: PreviewAudioDescriptor) => {
      ...
      installPlaybackRateState(rateState);
      const sourceUrl = descriptor.audioUrl;
      pendingStartRef.current = {
        sourceUrl,
        startSeconds: 0,
      };
  ...
  const setOutputEffects = useCallback(
    (partial: Partial<OutputEffectsState>) => {
      const previous = outputEffectsRef.current;
      const nextState: OutputEffectsState = {
        volumeBoost:
      ...
      outputEffectsRef.current = nextState;
      setOutputEffectsState(nextState);
      if (areOutputEffectsActive(nextState)) {
        resumeOutputEffectsGraphIfRequired();
      } else if (sourceNodeRef.current) {
        configureOutputEffectsGraph();
      }
      try {
        writeOutputEffectsToStorage(window.localStorage, nextState);
      } catch {
        // Ignore storage failures (private mode / quota).
      }
    },
    [configureOutputEffectsGraph, resumeOutputEffectsGraphIfRequired],
  );
    ...
    (node: HTMLAudioElement | null) => {
      ...
      if (node) {
        ...
        if (recoveringAudioElementRef.current) {
          ...
          const sourceUrl = pendingStartRef.current?.sourceUrl;
          const captureAllowed =
            sourceUrl === undefined ||
            canCaptureOutputEffectsSource(sourceUrl);
          setOutputEffectsAvailable(captureAllowed);
          outputEffectsAvailableRef.current = captureAllowed;
        }
      }
    },
    [applyCurrentPlaybackRate, resetAudioGraphNodes],
  );
  ...
  useEffect(
    () =>
      subscribePodcastSubscriptionSettingsInstalls((install) => {
        ...
        installedPodcastPreferencesRef.current.set(
          podcastId,
          install.kind === "Settings"
            ? {
                kind: "Active",
                value: install.settings.default_playback_speed,
              }
            : { kind: "Lapsed" },
        );
        const next: PlaybackRateState = {
          ...
          podcastPreference:
            install.kind === "Settings"
              ...
              : absent(),
          remember:
            install.kind === "Unsubscribed"
              ? { kind: "Unavailable" }
              : rememberAttemptRef.current?.podcastId === podcastId
              ? { kind: "Pending" }
              : { kind: "Ready" },
        };
  ...
  // --- Restore persisted volume / output effects -----------------------------
  ...
  useEffect(() => {
    try {
      const restored = readOutputEffectsFromStorage(window.localStorage);
      setOutputEffectsState(restored);
      outputEffectsRef.current = restored;
    } catch {
      // Ignore localStorage failures.
    }
  }, []);
  ...
  useEffect(() => {
    if (
      ...
      !outputEffectsAvailableRef.current
    ) {
      return;
    }
    configureOutputEffectsGraph();
  }, [configureOutputEffectsGraph, outputEffects]);
  ...
  const settings = useMemo<PlayerSettingsCapability>(
    () => ({
      ...
      playbackRate,
      outputEffects,
      outputEffectsAvailable,
      pauseShortening: {
        kind: "Unavailable",
        reason: "RuntimeUnsupported",
      },
    }),
    [outputEffects, outputEffectsAvailable, playbackRate, volume],
  );
