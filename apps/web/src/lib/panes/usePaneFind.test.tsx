...
describe("usePaneFind", () => {
  ...
  it.each(["Rejected", "Previewed"] as const)(
    ...
    async (lateKind) => {
      ...
      const settle = settleAutoPreview;
      if (!autoRequest || !settle) {
        throw new Error("Expected a pending auto-preview.");
      }
      await act(async () => {
        settle(previewReceipt(autoRequest, lateKind));
        await Promise.resolve();
      });
  ...
  it.each(["Rejected", "Previewed"] as const)(
    ...
    async (lateKind) => {
      ...
      const settle = settleExplicitPreview;
      if (!explicitRequest || !settle || !firstActivation) {
        throw new Error("Expected a pending explicit preview.");
      }
      await act(async () => {
        settle(previewReceipt(explicitRequest, lateKind));
