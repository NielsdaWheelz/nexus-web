...
import { expectArray, hasExactKeys } from "@/lib/validation";
import {
  RESULT_TYPE_VALUES,
...
function decodeSearchContributors(value: unknown): ContributorCredit[] {
  return expectArray(
    value,
    (raw, index) => decodeContributorCredit(raw, index, "Search contributors"),
    "Search contributors",
  );
}
