...
describe("library destination response contract", () => {
  ...
  it("posts the caller-owned replay identity in the strict create body", async () => {
    ...
    expect(apiFetchMock).toHaveBeenCalledWith("/api/libraries", {
      method: "POST",
      body: JSON.stringify({ library_id: libraryId, name: "Research" }),
      signal: controller.signal,
    });

    apiFetchMock.mockResolvedValueOnce({
      data: { ...library, id: "00000000-0000-4000-8000-000000000299" },
    });
    await expect(
