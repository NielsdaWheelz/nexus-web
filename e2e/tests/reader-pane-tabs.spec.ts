...
test.describe("reader pane tabs (evidence cutover)", () => {
  ...
  // the Evidence and Dossier surfaces and defaults to Evidence. This test pins
  // that behavior plus the removal of all five legacy reader-tools tabs.
  test("exposes the Evidence surface and none of the retired reader-tools tabs", async ({
    ...
    const tablist = secondary.getByRole("tablist", {
      name: "Secondary surfaces",
    });
    const tabs = tablist.getByRole("tab");
    const evidenceTab = tablist.getByRole("tab", { name: "Evidence" });

    // No ToC → Evidence + Dossier, with no empty Contents surface.
    await expect(tabs).toHaveCount(2);
    await expect(evidenceTab).toBeVisible();
    await expect(tablist.getByRole("tab", { name: "Dossier" })).toBeVisible();
    ...
    await expect(tablist.getByRole("tab", { name: "Highlights" })).toHaveCount(
      0,
    );
    await expect(tablist.getByRole("tab", { name: "Embeds" })).toHaveCount(0);
    await expect(tablist.getByRole("tab", { name: "Chat" })).toHaveCount(0);
    await expect(tablist.getByRole("tab", { name: "Citations" })).toHaveCount(
      0,
    );
    await expect(tablist.getByRole("tab", { name: "Connections" })).toHaveCount(
      0,
    );
