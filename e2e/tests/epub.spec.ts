...
async function upsertHighlightNote(
  ...
  const edgesResponse = await page.request.post(
...
async function fetchEpubNavigation(
  page: Page,
  mediaId: string,
): Promise<EpubNavigationResponse> {
...
async function findSectionByLabel(
  ...
  if (!section.href_path) {
    throw new Error(
      `Expected navigation section "${label}" to expose href_path.`,
    );
  }
  return {
...
async function ensureFragmentHighlight(
  ...
  if (createResponse.status() === 409) {
    ...
    const existing = payload.data.highlights.find(
      (item) =>
        item.anchor.start_offset === startOffset &&
        item.anchor.end_offset === endOffset,
    );
...
async function expectHighlightRowVisible(
  ...
  await expect
    .poll(() => rowContainsVisibleTextOrFieldValue(row, noteText), {
      timeout: 10_000,
    })
    .toBe(true);
  const page = row.page();
  ...
  await expect(
    page.getByRole("menuitem", { name: "Delete highlight" }),
  ).toBeVisible();
}
...
async function readAnchorCenteringError(
  ...
  return activeWorkspacePane(page).evaluate((pane, id) => {
    const contentRoot = pane.querySelector<HTMLElement>(
      'div[class*="fragments"]',
    );
    if (!contentRoot) {
      return null;
    ...
    const idealScrollTop = Math.min(
      maxScrollTop,
      Math.max(0, anchorCenterInScrollContent - scroller.clientHeight / 2),
    );
...
async function expectLocatorInReaderViewport(locator: Locator): Promise<void> {
  await expect
    .poll(() => isLocatorInReaderViewport(locator), { timeout: 10_000 })
    .toBe(true);
}
...
async function wheelUntilLocatorInViewport(
  ...
  const contentRoot = activeWorkspacePane(page)
    .locator('div[class*="fragments"]')
    .first();
...
async function selectSectionByLabel(page: Page, label: string): Promise<void> {
  ...
  await expect(
    sectionSelect.locator("option").filter({ hasText: label }),
  ).toHaveCount(1, {
    timeout: 10_000,
  });
...
async function clickToolbarAction(
  ...
  const overflowToggle = activePane
...
test.describe("epub", () => {
  ...
  test("upload EPUB", async ({ page }, testInfo) => {
    ...
    const uploadButton = addContentPanel.getByRole("button", {
      name: "Choose PDF or EPUB",
    });
  ...
  test("renders EPUB image assets through the BFF when present", async ({
    ...
    for (const image of imageStates) {
      ...
      expect(image.resolvedSrc).toContain(
        `/api/media/${seed.media_id}/assets/`,
      );
    }
  ...
  test("publisher CSS does not affect EPUB reader chrome", async ({
    ...
    await expect
      .poll(async () =>
        renderer.evaluate((root) => ({
          ...
          stylesheetCount: root.querySelectorAll(
    ...
    const chrome = activePane
      .locator('[data-testid="pane-shell-chrome"]')
      .first();
    await expect(chrome).toBeVisible();
    const chromeState = await chrome.evaluate((element) => {
      const rect = element.getBoundingClientRect();
      const rendererRoot = document.querySelector(
  ...
  test("navigate sections", async ({ page }, testInfo) => {
    ...
    await expect
      ...
      .toBeGreaterThanOrEqual(seed.chapter_count);
    await expect
      .poll(async () => {
        const optionLabels = await options.allTextContents();
        return seed.chapter_titles.every((title) =>
          optionLabels.includes(title),
        );
      })
      .toBe(true);
  });
  ...
  test("saved EPUB resume locator wins over a cold loc query and strips it from the URL", async ({
    ...
    const secondSection = await findSectionByLabel(
      page,
      seed.media_id,
      seed.chapter_titles[1],
    );

    await putReaderState(
      page,
      seed.media_id,
      buildEpubReaderState(secondSection),
    );
    ...
    let activePane = await gotoEpubReader(
      page,
      testInfo,
      seed.media_id,
      firstSection.section_id,
    ...
    if (
      savedSnapshot.state !== "Positioned" ||
      !isEpubReaderResumeState(savedSnapshot.locator)
    ) {
  ...
  test("manual scroll before delayed EPUB restore settles does not snap back late", async ({
    ...
    const sectionDetail = await fetchEpubSectionDetail(
      page,
      seed.media_id,
      firstSection.section_id,
    );
    const restoreQuote = "introduction chapter of the E2E test EPUB";
    const manualScrollQuote =
      "Deterministic post-anchor filler paragraph 8 for E2E.";
    const restoreOffset =
      sectionDetail.data.canonical_text.indexOf(restoreQuote);
    ...
    const manualScrollTarget = activePane
      .getByText(manualScrollQuote, { exact: true })
      .first();
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      expect(currentScrollTop ?? 0).toBeGreaterThan(
        (manualScrollTop ?? 0) - 120,
      );
    }
  ...
  test("create highlight in epub", async ({ page }, testInfo) => {
    ...
    const section = await fetchEpubSectionDetail(
      page,
      seed.media_id,
      firstSection.section_id,
    );
    ...
    const existingExacts = [
      ...existingHighlightsPayload.data.highlights.map(
    ...
    const highlightedSegments = activePane.locator(
      '[class*="fragments"] [data-active-highlight-ids]',
    );
  ...
  test("document-wide highlights expand inline while context and source focus stay in sync", async ({
    ...
    const chapter1SecondaryNeedle =
      "Deterministic post-anchor filler paragraph 8 for E2E.";
    const chapter1PrimaryStart = firstSectionDetail.data.canonical_text.indexOf(
    ...
    const chapter2Start =
      secondSectionDetail.data.canonical_text.indexOf(chapter2Needle);
    ...
    const chapter1SecondaryAnchor = activePane
      .locator(
        `[data-active-highlight-ids~="${chapter1SecondaryHighlight.id}"]`,
      )
      .first();
    ...
    await expect(chapter2Row).toContainText("EPUB chapter two inspector note.");
    ...
    await expect(
      activePane.getByRole("dialog", { name: /highlight details/i }),
    ).toHaveCount(0);
    await expect(
      activePane.getByRole("button", { name: /show in document/i }),
    ).toHaveCount(0);
    ...
    await expect
      .poll(
        async () =>
          (await readAnchorCenteringError(
            page,
            chapter1SecondaryHighlight.id,
          )) ?? Number.POSITIVE_INFINITY,
        { timeout: 15_000 },
