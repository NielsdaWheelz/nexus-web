...
async function uploadPdf(
  ...
  const panel = await openAddContentPanel(page);
  const input = panel.getByLabel("Choose PDF or EPUB files");
  await expect(input).toBeAttached();
  await input.setInputFiles(fixturePath);
  const submit = panel.getByRole("button", { name: "Add 1 item" });
...
test("Reading Slate acceptance preserves survivors, excludes the accepted target, and reconciles on library reactivation", async ({
  ...
  try {
    ...
    const addAnchor = await page.request.post(
      `/api/media/${anchorMediaId}/libraries`,
      {
        headers: stateChangingApiHeaders(),
        data: { library_ids: [libraryId] },
      },
    );
    expect(addAnchor.status()).toBe(204);

    for (const candidateId of candidateMediaIds) {
      const createLink = await page.request.post("/api/resource-graph/links", {
        headers: stateChangingApiHeaders(),
        data: {
          client_mutation_id: `slate-${anchorMediaId}-${candidateId}`,
          source: { kind: "resource", ref: `media:${anchorMediaId}` },
          target: { kind: "resource", ref: `media:${candidateId}` },
        },
      });
      await expectOk(createLink, `Link Slate candidate ${candidateId}`);
      linkIds.push(
        (
          (await createLink.json()) as {
            data: { connection: { edge_id: string } };
          }
        ).data.connection.edge_id,
      );
    }
    ...
    for (const linkId of linkIds.reverse()) {
      try {
        await deleteE2eResource(
          page.request,
          `/api/resource-graph/links/${linkId}`,
          `Slate Link ${linkId}`,
        );
      } catch (error) {
