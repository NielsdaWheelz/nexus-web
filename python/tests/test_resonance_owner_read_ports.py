...
def test_hidden_episode_facts_do_not_boost_podcast_resonance_or_anchor_rank(
    db_session: Session, bootstrapped_user: UUID
) -> None:
    library_id = create_test_library(
        db_session,
        bootstrapped_user,
        "Visibility-scoped podcast engagement",
    )
    ...
    future_engagement_episode_id = uuid4()
    article_id = uuid4()
    db_session.add_all(
        [
            Podcast(
                id=podcast_id,
                provider="podcast_index",
                provider_podcast_id=f"visibility-{podcast_id}",
                title="Visibility-scoped podcast",
                feed_url=f"https://example.com/{podcast_id}.xml",
            ),
            Media(
                id=visible_episode_id,
                kind=MediaKind.podcast_episode.value,
                title="Visible older episode",
                processing_status=ProcessingStatus.ready_for_reading,
            ),
            Media(
                id=hidden_episode_id,
                kind=MediaKind.podcast_episode.value,
                title="Hidden recently engaged episode",
                processing_status=ProcessingStatus.ready_for_reading,
            ),
            Media(
                id=future_engagement_episode_id,
                kind=MediaKind.podcast_episode.value,
                title="Visible future-engagement episode",
                processing_status=ProcessingStatus.ready_for_reading,
            ),
            Media(
                id=article_id,
                kind=MediaKind.web_article.value,
                title="Middle-aged visible article",
                processing_status=ProcessingStatus.ready_for_reading,
    ...
    db_session.add_all(
        [
            PodcastEpisode(
                ...
                podcast_id=podcast_id,
                provider_episode_id=f"visible-{visible_episode_id}",
                fallback_identity=f"visible-{visible_episode_id}",
                published_at=old,
            ),
            PodcastEpisode(
                media_id=hidden_episode_id,
                podcast_id=podcast_id,
                provider_episode_id=f"hidden-{hidden_episode_id}",
                fallback_identity=f"hidden-{hidden_episode_id}",
                published_at=recent,
            ),
            PodcastEpisode(
                media_id=future_engagement_episode_id,
                podcast_id=podcast_id,
                provider_episode_id=f"future-{future_engagement_episode_id}",
                fallback_identity=f"future-{future_engagement_episode_id}",
                published_at=old,
            ),
            PodcastSubscription(
                user_id=bootstrapped_user,
    ...
    db_session.flush()
    add_media_to_library(db_session, default_library_id, visible_episode_id)
    add_media_to_library(db_session, default_library_id, hidden_episode_id)
    add_media_to_library(
        db_session,
        default_library_id,
        future_engagement_episode_id,
    )
    db_session.add_all(
        [
            UserMediaDeletion(user_id=bootstrapped_user, media_id=hidden_episode_id),
            PodcastListeningState(
                user_id=bootstrapped_user,
                media_id=hidden_episode_id,
                position_ms=60_000,
                duration_ms=300_000,
                last_engaged_at=recent,
                updated_at=recent,
            ),
            PodcastListeningState(
                user_id=bootstrapped_user,
    ...
    assert [fact.ref.uri for fact in anchors] == [f"media:{article_id}"]

    entries, page = resonance_service.rank_library_entry_page(
        db_session,
        viewer_id=bootstrapped_user,
        library_id=library_id,
        limit=2,
    ...
    assert [entry.kind for entry in entries] == ["media", "podcast"]
    assert entries[0].media is not None
    assert entries[0].media.id == article_id
    assert entries[1].podcast is not None
    assert entries[1].podcast.id == podcast_id

    valid_past_engagement = as_of - timedelta(days=2)
    db_session.add(
        PodcastListeningState(
            user_id=bootstrapped_user,
            media_id=visible_episode_id,
            position_ms=60_000,
            duration_ms=300_000,
            last_engaged_at=valid_past_engagement,
            updated_at=valid_past_engagement,
        )
    )
    db_session.commit()
    ...
    assert [fact.ref.uri for fact in anchors] == [f"podcast:{podcast_id}"]
    ...
    assert page.has_more is False
    assert [entry.kind for entry in entries] == ["podcast", "media"]
    assert entries[0].podcast is not None
    assert entries[0].podcast.id == podcast_id
...
def test_library_resonance_keeps_finite_nonrecency_score_when_all_dates_are_future(
    db_session: Session, bootstrapped_user: UUID
) -> None:
    library_id = create_test_library(
        db_session,
        bootstrapped_user,
        "Future-fact Resonance",
    )
    default_library_id = _default_library(db_session, bootstrapped_user)
    as_of = db_session.execute(text("SELECT now()")).scalar_one()
    future = as_of + timedelta(days=30)
    media_ids = [uuid4() for _ in range(3)]
    db_session.add_all(
        [
            Media(
                id=media_id,
                kind=MediaKind.web_article.value,
                title=f"Future-dated member {index}",
                processing_status=ProcessingStatus.ready_for_reading,
                created_by_user_id=bootstrapped_user,
            )
    ...
    db_session.add_all(
        [
            Contributor(
                id=contributor_id,
                handle=f"future-author-{contributor_id.hex[:12]}",
                display_name=f"Future Author {index}",
            )
            for index, contributor_id in enumerate(contributor_ids)
        ]
    )
    credit_pairs = [
        (media_ids[0], contributor_ids[0], 0),
        (media_ids[0], contributor_ids[1], 1),
        (media_ids[1], contributor_ids[0], 0),
        (media_ids[2], contributor_ids[1], 0),
    ]
    db_session.add_all(
        [
            ContributorCredit(
                ...
                contributor_id=contributor_id,
                media_id=media_id,
                credited_name="Future Author",
                normalized_credited_name=contributor_match_key("Future Author"),
                role="author",
                ordinal=ordinal,
                source="manual",
            )
            for media_id, contributor_id, ordinal in credit_pairs
        ]
    )
    db_session.commit()
    ...
    assert page.has_more is False
    assert entries[0].kind == "media"
    assert entries[0].media is not None
    assert entries[0].media.id == media_ids[0]
    assert {
        entry.media.id for entry in entries if entry.kind == "media" and entry.media is not None
    } == set(media_ids)
