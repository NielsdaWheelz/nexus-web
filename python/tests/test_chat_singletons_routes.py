...
def test_get_singleton_media_null_when_absent(auth_client, direct_db: DirectSessionManager):
    ...
    response = auth_client.get(f"/chat-singletons/media/{media_id}", headers=auth_headers(user_id))
...
def test_get_singleton_library_null_when_absent(auth_client, direct_db: DirectSessionManager):
    user_id = create_test_user_id()
    auth_client.get("/me", headers=auth_headers(user_id))
    with direct_db.session() as session:
...
def test_get_singleton_forbidden_when_media_invisible(auth_client, direct_db: DirectSessionManager):
    ...
    with direct_db.session() as session:
        ...
        media_id = create_test_media_in_library(session, owner_id, library_id, title="Private Doc")
    direct_db.register_cleanup("library_entries", "media_id", media_id)
    direct_db.register_cleanup("media", "id", media_id)
    direct_db.register_cleanup("memberships", "library_id", library_id)
    direct_db.register_cleanup("libraries", "id", library_id)

    response = auth_client.get(f"/chat-singletons/media/{media_id}", headers=auth_headers(other_id))

    assert response.status_code == 403, response.text
    assert response.json()["error"]["code"] == "E_SINGLETON_TARGET_FORBIDDEN"


def test_get_singleton_media_404_when_missing(auth_client, direct_db: DirectSessionManager):
