...
def mock_rate_limiter():
    """Create a mock rate limiter that doesn't use Redis."""
    limiter = RateLimiter(
        redis_client=None,
        rpm_limit=20,
        concurrent_limit=3,
...
def create_test_model(session: Session) -> UUID:
    ...
    session.execute(
        text("""
            INSERT INTO models (id, provider, model_name, display_name, is_available)
            VALUES (:id, 'openai', 'gpt-4o', 'GPT-4o', true)
...
def create_test_media(
    ...
    session.execute(
        text("""
            INSERT INTO media (id, library_id, kind, title, canonical_url, processing_status)
            VALUES (:id, :library_id, 'web_article', :title, 'https://example.com/article', :status)
        """),
        {"id": media_id, "library_id": library_id, "title": title, "status": status},
    )
    session.commit()
    return media_id
...
def create_test_fragment(
    ...
    session.execute(
        text("""
            INSERT INTO fragments (id, media_id, ordinal, content_text, fragment_block)
            VALUES (:id, :media_id, 0, :content, :block)
        """),
        {"id": fragment_id, "media_id": media_id, "content": content, "block": content[:100]},
    )
    session.commit()
    return fragment_id
...
class TestSendMessageBasic:
    ...
    def test_send_message_creates_conversation_and_messages(
        ...
        direct_db.register_cleanup("models", "id", model_id)
...
class TestSendMessageLLMErrors:
    ...
    def test_llm_invalid_key_marks_byok_invalid(
        ...
        with direct_db.session() as session:
            ...
            session.execute(
                text("""
                    INSERT INTO user_api_keys (id, user_id, provider, status, encrypted_key, key_nonce)
                    VALUES (:id, :user_id, 'openai', 'untested', 'encrypted', 'nonce')
                """),
