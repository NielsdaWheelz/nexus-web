...
def test_each_browser_capability_retires_only_process_liveness(tmp_path: Path) -> None:
    ...
    fixture = runner.ProviderApiPeer(
        tmp_path / "provider-api-peer",
        tmp_path / "provider-api-peer/ca.pem",
        tmp_path / "provider-api-peer/server-key.pem",
        tmp_path / "provider-api-peer/requests.jsonl",
        19092,
    )
    embedding = runner.EmbeddingPeer(
        tmp_path / "embedding-peer",
        tmp_path / "embedding-peer/ca.pem",
        tmp_path / "embedding-peer/server-key.pem",
        tmp_path / "embedding-peer/requests.jsonl",
        19093,
    )
    generation = runner.CodexGenerationPeer(
        tmp_path / "generation-peer",
        tmp_path / "generation-peer/agent.sock",
        tmp_path / "generation-peer/requests.jsonl",
    )
    execution = runner._WorkflowExecution(
        CapabilityContext(tmp_path, Workflow.FULL, ()),
        {},
        ...
        provider_api_peer=fixture,
        embedding_peer=embedding,
        generation_peer=generation,
        browser_data_plane_prepared=True,
        provider_api_protocol_started=True,
    ...
    for capability in (Capability.JOURNEYS_ALL, Capability.EXTENSION):
        ...
        assert not execution.provider_api_protocol_started
        assert not execution.journey_runtime_started
        assert execution.provider_api_peer is fixture
        assert execution.embedding_peer is embedding
        assert execution.generation_peer is generation
        assert execution.browser_data_plane_prepared
        execution.external_protocol_started = True
        execution.provider_api_protocol_started = True
...
def test_browser_runtime_retirement_failure_fails_the_capability_and_keeps_liveness(
    ...
    assert execution.provider_api_protocol_started
