...
def test_cancelled_recovery_ignores_unavailable_provider_admission(
    engine: Engine, committed_chat_state_isolation: None
) -> None:
    """An unavailable provider cannot strand cancellation of an accepted generation."""
...
async def _prove_cancellation_without_dispatch_admission(engine: Engine) -> None:
    ...
    with factory() as db:
        ...
        cancel_chat_run(db, viewer_id=chat.user_id, run_id=chat.run_id, catalog_snapshot=snapshot)
        ...
        assert read_model_turns(db, generation_id=recovered.generation_id) == original_children
        assert (
            db.scalar(
                select(func.count())
