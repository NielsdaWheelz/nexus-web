...
from tests.testkit.unreachable_state import expire_claim_and_handoff_code, expire_job_claim
...
def _wait_for_job(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        observed = tuple(row)
        if row.status == status and row.attempts == attempts:
            return observed
...
def _assert_production_worker(
    ...
    assert environment.get("WORKER_LANE") == "background"
...
def test_owned_worker_replays_committed_note_index_after_process_death(
    ...
    try:
        ...
        claimed = _wait_for_job(engine, job_id, status="running", attempts=1)
