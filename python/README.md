...
│   └── llm/             # LLM adapter layer (S3 PR-05)
│       ├── types.py     # LLM data types (request, response, error)
│       ├── adapter.py   # Abstract base adapter class
│       ├── openai_adapter.py    # OpenAI API adapter
│       ├── anthropic_adapter.py # Anthropic API adapter
│       ├── gemini_adapter.py    # Gemini API adapter
│       ├── router.py    # Key resolution and adapter dispatch
│       └── prompt.py    # System prompt and context rendering
