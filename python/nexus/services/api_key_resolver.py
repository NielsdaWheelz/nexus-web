...
Resolution behavior:
- key_mode="auto": Use BYOK when present, otherwise use the platform key
...
from nexus.llm_catalog import VALID_KEY_MODES, is_provider_enabled, platform_key_for_provider
...
def resolve_api_key(
    ...
    Raises:
        ModelCallError: If no key is available.
    """
    ...
    if key_mode not in VALID_KEY_MODES:
        raise ApiError(
            ...
            f"Invalid LLM key mode: {key_mode}",
        )
    ...
    if key_mode == "byok_only":
        ...
        raise ModelCallError(
            error_code=ModelCallErrorCode.INVALID_KEY,
