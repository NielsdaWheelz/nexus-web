...
def resolve_highlight_search_result(
    db: Session,
    *,
    viewer_id: UUID,
    result_id: UUID,
    ...
    """Rematerialize one visible, indexed Highlight with a valid typed anchor."""
    row = db.execute(
        text(
            f"""
            WITH
...
def _highlight_locator(row: Row[Any]) -> dict[str, Any] | None:
    ...
    if row[10] != "pdf_page_geometry" or row[17] is None:
        return None
    try:
