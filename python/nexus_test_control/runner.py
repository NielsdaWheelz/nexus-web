...
class _WorkflowExecution:
    ...
    provider_api_protocol_started: bool = False
    embedding_peer: EmbeddingPeer | None = None
    generation_peer: CodexGenerationPeer | None = None
    ...
    def ensure_provider_api_protocol(
        ...
        if self.provider_api_protocol_started:
            return None
        try:
            peer = self.provider_api_peer
            if peer is None:
        ...
        self.provider_api_protocol_started = True
        return None
    ...
    def retire_browser_runtime(self) -> None:
        ...
        self.provider_api_protocol_started = False
    ...
    def results() -> Iterable[CapabilityResult]:
        ...
        try:
            for requirement in requirements:
                ...
                result = _retire_browser_runtime_after_capability(
                    requirement.capability, result, execution
                )
...
def _ensure_browser_processes(
    ...
    try:
        ...
        if provider_failure is not None:
            return provider_failure
        embedding_peer = execution.embedding_peer
        if embedding_peer is None:
            embedding_peer = execution.ports.materialize_embedding_peer(
                context.repo_root,
                {"NEXUS_ENV": "test"},
                prepared,
            )
            execution.embedding_peer = embedding_peer
        ...
        generation_peer = execution.generation_peer
        if generation_peer is None:
            generation_peer = execution.ports.materialize_generation_peer(
                context.repo_root,
                {"NEXUS_ENV": "test"},
                prepared,
            )
            execution.generation_peer = generation_peer
