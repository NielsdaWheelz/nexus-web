...
def _stop_run_process(root: Path, candidate: CleanupCandidate, run_id: str) -> None:
    ...
    if candidate.resource.identity == process_resource_identity(run_id, "codex-generation-peer"):
        paths = _owned_codex_generation_peer_paths(root, run_id, require_socket=None)
        paths[_CODEX_GENERATION_PEER_SOCKET].unlink(missing_ok=True)
    ...
    with run_lifecycle_lock(root, environment, run_id):
        ...
        for candidate in cleanup_candidates(root, environment, run_id):
            if candidate.resource.kind is not ResourceKind.PROCESS:
                continue
            try:
                _stop_run_process(root, candidate, run_id)
