...
def _promoted_targets(path: str) -> tuple[SelectionTarget, ...]:
    if path == "python/tests/conftest.py":
        return tuple(
            SelectionTarget(capability) for capability in _GLOBAL_PYTEST_SUPPORT_CAPABILITIES
        )
    python_owner = _python_proof_owner(path)
    if python_owner is not None and path.endswith(".py") and not _is_python_test_file(path):
        return (SelectionTarget(python_owner),)
    if path in {
...
def _python_proof_owner(path: str) -> Capability | None:
    for prefix, capability in _PYTHON_PROOF_OWNERS:
